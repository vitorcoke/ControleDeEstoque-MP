...
const getById = async (id: number) => {
  try {
    const urlRelativa = `/notebooksSP/${id}`;
    const {data} = await Api.get(urlRelativa);
...
const create = async (notebooks: INotebook) => {
  try {
    const {data} = await Api.post('/notebooksSP', notebooks);
    if(data){
      return data;
    }
...
export default notebooksSPServices;