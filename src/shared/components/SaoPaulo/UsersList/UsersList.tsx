...
const UsersList = () => {
  ...
  const columns = [
    { field: 'col1', headerName: 'Nome', width: 580 },
    { field: 'col2', headerName: 'Departamento', width: 575 },
  ];

  const rows = Users.map(user => (
    { id: user.id, col1: user.name, col2: user.departament }
  ));
  ...
  return (
    <Box display='flex' width='100%' justifyContent='center' >
      <Box component={Paper} sx={{ height: 632, width: '75%' }}>
        <DataGrid
          rows={rows}
          columns={columns}
          pageSize={10}
